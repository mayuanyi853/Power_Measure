...
unsigned  char	 xdata  Measure_Buf[6][4]  = {
											   0x00 ,0x00 ,0x00 ,0x00 ,	   //1路电压  值 电流 ，值	功率
                                               0x00 ,0x00 ,0x00 ,0x00 ,
										       0x00 ,0x00 ,0x00 ,0x00 ,
										       0x00 ,0x00 ,0x00 ,0x00 ,
										       0x00 ,0x00 ,0x00 ,0x00 ,
...
unsigned  int xdata    measure_ma[6][2]={
										    	0x0000 ,0x0000 ,	   //1路电压  ma电流 ，ma	
                                                0x0000 ,0x0000 ,
										        0x0000 ,0x0000 ,
										        0x0000 ,0x0000 ,
										        0x0000 ,0x0000 ,
	...
	if( DRDY_Flag == 1 ){ // 读取当前转换结果		  	
		...
		if( Read_State >= 3 ){ Read_State = 0;
		  ...
		  //	Temp_l  = Temp_f;			// 转换为长整形数
				...
				measure_ma[CS5460_Channel][0]=Temp_l;// cun ri ma
		...
		//	LongToASCII( &Measure_Buf[CS5460_Channel][0][0], Temp_l, 'V' );
             ...
             Measure_Buf[CS5460_Channel][0]=Temp_l>>8;
		...
		//	LongToASCII( &Measure_Buf[CS5460_Channel][1][0], Temp_l, 'I' );
           ...
           Measure_Buf[CS5460_Channel][2]=Temp_l>>8;  //电流码
	 ...
	 unsigned  int	  ee_address;
		 ...
		 ee_address=0x200;
	 	 ...
	 	 for(i=0; i<=5; i++)
	  	  ...
	  	  ISP_Byte_Write(0x212 ,0x01);	 //eeprom   212存入01h
